...
const post = (props) => {
        ...
        if (props.comments && Object.keys(props.comments).length) {
            postsComments = Object.keys(props.comments).map(key => ({...props.comments[key]}))
                ...
                <Comment
                    ...
                    key={comment.id}
                    id={comment.id}
                    commentUserKey={comment.userKey}
    ...
    let profileSummary;
    if (viewingSummary) {
        profileSummary = <ProfileSummary loc="POST" userType={props.userType} userKey={props.userKey} publicProfileKey={props.publicProfileKey} myFriends={props.friends} onMouseEnter={cancelCloseSummary} onMouseLeave={startClosingViewingSummary}/>
    }
